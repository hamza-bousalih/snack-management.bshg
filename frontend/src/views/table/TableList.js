...
import TableCreate from "src/views/table/TableCreate";
...
const TableList = () => {
  ...
  const paginate = (page, size) => {
    setPaginating(true)
    Services.tableService.findPaginated(page, size)
  ...
  const deleteHandler = () => {
    Services.tableService.deleteById(currentId)
      .then(() => {
        setPagination(prev => {
              ...
              <CTable hover={true} responsive={true} striped={true} align="middle" className="mb-0 border">
                <CTableHead className='text-nowrap text-truncate'>
                  <CTableRow>
                    ...
                    <CTableHeaderCell className="bg-body-tertiary">TableNumber</CTableHeaderCell>
                ...
                <CTableBody>
                  {pagination.data.map((it, i) =>
                    <CTableRow key={i}>
                      ...
                      <CTableDataCell>{ it.tableNumber }</CTableDataCell>
