...
import UserCreate from "src/views/user/UserCreate";
import UserUpdate from "src/views/user/UserUpdate";
...
const UserList = () => {
  ...
  const paginate = (page, size) => {
    setPaginating(true)
    Services.userService.findPaginated(page, size)
  ...
  const deleteHandler = () => {
    Services.userService.deleteById(currentId)
      .then(() => {
        setPagination(prev => {
              ...
              <CTable hover={true} responsive={true} striped={true} align="middle" className="mb-0 border">
                <CTableHead className='text-nowrap text-truncate'>
                  <CTableRow>
                    ...
                    <CTableHeaderCell className="bg-body-tertiary">Fullname</CTableHeaderCell>
                ...
                <CTableBody>
                  {pagination.data.map((it, i) =>
                    <CTableRow key={i}>
                      ...
                      <CTableDataCell>{ it.fullname }</CTableDataCell>
