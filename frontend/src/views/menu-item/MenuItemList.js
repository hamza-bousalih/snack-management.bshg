...
const MenuItemList = () => {
  ...
  const paginate = (page, size) => {
    setPaginating(true)
    Services.menuItemService.findPaginated(page, size)
  ...
  const deleteHandler = () => {
    Services.menuItemService.deleteById(currentId)
      .then(() => {
        setPagination(prev => {
              ...
              <CTable hover={true} responsive={true} striped={true} align="middle" className="mb-0 border">
                <CTableHead className='text-nowrap text-truncate'>
                  <CTableRow>
                    ...
                    <CTableHeaderCell className="bg-body-tertiary">Catrgory</CTableHeaderCell>
                ...
                <CTableBody>
                  {pagination.data.map((it, i) =>
                    <CTableRow key={i}>
                      ...
                      <CTableDataCell>{ it.catrgory }</CTableDataCell>
