...
import BillCreate from "src/views/bill/BillCreate";
import BillUpdate from "src/views/bill/BillUpdate";
...
const BillList = () => {
  ...
  const paginate = (page, size) => {
    setPaginating(true)
    Services.billService.findPaginated(page, size)
  ...
  const deleteHandler = () => {
    Services.billService.deleteById(currentId)
      .then(() => {
        setPagination(prev => {
              ...
              <CTable hover={true} responsive={true} striped={true} align="middle" className="mb-0 border">
                <CTableHead className='text-nowrap text-truncate'>
                  <CTableRow>
                    ...
                    <CTableHeaderCell className="bg-body-tertiary">TotalAmount</CTableHeaderCell>
                    <CTableHeaderCell className="bg-body-tertiary">Discount</CTableHeaderCell>
                    <CTableHeaderCell className="bg-body-tertiary">Tax</CTableHeaderCell>
                    <CTableHeaderCell className="bg-body-tertiary">FinalAmmount</CTableHeaderCell>
                ...
                <CTableBody>
                  {pagination.data.map((it, i) =>
                    <CTableRow key={i}>
                      ...
                      <CTableDataCell>{ it.totalAmount }</CTableDataCell>
                      <CTableDataCell>{ it.discount }</CTableDataCell>
                      <CTableDataCell>{ it.tax }</CTableDataCell>
                      <CTableDataCell>{ it.finalAmmount }</CTableDataCell>
                      <CTableDataCell>{ it.createdAt }</CTableDataCell>
                      <CTableDataCell>{ it.order ? it.order.id: "null" }</CTableDataCell>
                      <CTableDataCell>{ it.issuer ? it.issuer.id: "null" }</CTableDataCell>
